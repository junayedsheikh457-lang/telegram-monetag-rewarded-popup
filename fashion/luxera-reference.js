(function(){
  ...
  function addRelatedProducts(modal,currentId){
    ...
    var box=modal.querySelector('.box');if(!box)return;
    var all=[].slice.call(document.querySelectorAll('#grid .card,#luxeraMoreProducts .card'));
    var related=all.filter(function(c){var m=(c.getAttribute('onclick')||'').match(/detail\((\d+)\)/);return m&&Number(m[1])!==Number(currentId)}).slice(0,8);
    if(!related.length)return;
    var sec=document.createElement('section');sec.className='detail-related';sec.innerHTML='<div class="head"><h2>You May Also Like</h2><span class="view">More</span></div><div class="detail-related-grid"></div>';
    var rg=sec.querySelector('.detail-related-grid');
    related.forEach(function(originalCard){
      var card=originalCard.cloneNode(true);card.classList.add('detail-related-card');card.removeAttribute('onclick');
      ...
      var heart=card.querySelector('.heart');if(heart){heart.removeAttribute('onclick');heart.addEventListener('click',function(e){e.stopPropagation();var m=(originalCard.innerHTML.match(/toggleWish\((\d+)\)/)||[])[1];if(m&&typeof window.toggleWish==='function')window.toggleWish(Number(m))})}
      rg.appendChild(card);
    });box.appendChild(sec);
  }
  function hookDetail(){
    if(typeof window.detail!=='function')return false;if(window.__luxeraDetailHooked)return true;
    var original=window.detail;
    window.detail=function(id){original(id);setTimeout(function(){var modal=document.querySelector('.modal.show');addRelatedProducts(modal,id)},120)};
    window.__luxeraDetailHooked=true;return true;
  }
  function start(){arrange();hookDetail();setTimeout(arrange,600);setTimeout(hookDetail,600);setTimeout(arrange,1600);setTimeout(hookDetail,1600)}
